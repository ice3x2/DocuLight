...
const dataDir = await mkdtemp(join(tmpdir(), 'doculight-issue56-'));
let server;
let web;
...
try {
  ...
  const { installSession } = await verified.json(); const suffix = Date.now(); const user = `issue56-${suffix}`; const password = `Issue56-${suffix}-pass`;
  const installed = await fetch('http://127.0.0.1:3400/api/install/commit', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ installSession, superuserName: user, password, workspaceName: 'Issue 56 fixture', defaultGroupLevel: 'view', signupMode: 'approval' }) });
  ...
  const check = spawn(process.execPath, ['test/version-history-product-check.mjs'], { cwd: webDir, env: { ...process.env, DOCULIGHT_E2E_USER: user, DOCULIGHT_E2E_PASS: password }, stdio: 'inherit', windowsHide: true });
  const exitCode = await new Promise((resolveExit) => check.once('exit', resolveExit)); if (exitCode !== 0) process.exitCode = exitCode ?? 1;
