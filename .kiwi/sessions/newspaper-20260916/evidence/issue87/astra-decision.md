...
On ready L2 show the authoritative zero and no typing field. L3 requires exact plain total digits; no localized grouping, whitespace coercion, previous token or fuzzy comparison. HTTP transport outer optional whitespace (OWS) removed by the HTTP parser is canonical field-value equivalence, not application whitespace coercion: the server compares the parsed field value exactly and must not apply trim or numeric conversion. The UI still rejects leading or trailing whitespace and sends only the canonical digits. Title `보존 기간 축소`, action `변경 저장`, safe action `계속 편집`. Always show both proposed retention values, relevant typed breakdown and the FR-CONFIRM-008 warning: saving changes policy now; existing data is not deleted by clicking Save and will be removed at the next cleanup. Audit expiry is the permitted automatic removal path, not an audit-row edit/delete feature.
