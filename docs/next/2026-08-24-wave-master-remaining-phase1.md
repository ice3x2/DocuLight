...
### 8.6.2 `--max` 는 붙이지 않는다 — 사용자 결정

**2026-08-25 사용자 지시로 `--max` 를 뺐다.** 되살리지 마라.

`--max` 는 모든 wave 의 `/kiwi-pipeline` 과 하위 스킬 다섯(`kiwi-srs` · `kiwi-srs-feasibility` · `kiwi-planner` · `kiwi-pm` · `kiwi-review-fix-loop`)에 전파되며, `kiwi-review-fix-loop` 에서 까칠 리뷰어가 순차 2라운드로 늘고 비용이 2배가 된다(그 스킬 §1.3).

⚠️ **이 선택의 대가를 적어 둔다.** 검증은 1라운드로 돈다. 이 문서를 만든 2026-08-24 세션에서 **1라운드가 놓치고 2라운드가 잡은 것이 셋 있었다** — 새로 쓴 시험 둘이 아무것도 재지 않았다는 사실, `--req-filter` 가 feasibility 단계까지 흐르지 않아 `phase-1` 253건 전수에 `update_stability` 가 적용된다는 사실, `--drive` 로도 열리지 않는 게이트 목록을 잘못 옮긴 것. 1라운드로 끝냈으면 셋 다 살아남았다.

그러므로 **무인 구간에서 나온 산출물을 그대로 신뢰하지 말고**, 최종 검증 `pass` 뒤에 「다음 세션 지시서」 7번의 완료 조건 셋을 사람이 직접 대조하라. 비용이 다시 문제되지 않는다면 `--max` 를 되살리는 것이 이 대가를 없애는 방법이다.
...
### 8.6.4 `--drive` 로도 멈추는 지점 (닫힌 목록 — 사실이다, 희망이 아니다)
...
### 8.6.6 무인 완주가 성립하지 않는다고 판단되면

전제 조건 넷 중 하나라도 못 갖췄으면 **`--drive` 를 빼고 `--auto` 로 시작하라.** 그 편이 밤새 돈 뒤 「성공처럼 보이는 축소」를 받는 것보다 낫다.
