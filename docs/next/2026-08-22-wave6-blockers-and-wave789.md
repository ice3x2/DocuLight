...
- [x] B 단계 — 공허했던 판정 축 넷을 실제로 재게 함 — 커밋 `3e4f477`
- [x] 감사 보존 만료 소멸 구현 + 부등식 불변식 — 커밋 `fdbab79`
- [x] 뮤테이션으로 찾은 중복 제거(보존 경계 계산 일원화) — 커밋 `4fcb195`
- [x] 적대 검증 HIGH 1 · MEDIUM 3 · LOW 5 반영 — 커밋 `3deaf0d`
- [x] ACL·인증·주체 16건 검증 증거 — 커밋 `4db971a`
...
`packages/editor` 는 커밋 `a09cb3e` 직전 실행에서 237 passed / 1 skipped 였고, 그 커밋은 `packages/server` 만 건드렸다.
...
1. **wave 착수 순서는 7 → 8 → 9** — **확정**. (사용자 지시 2026-08-22. wave-6 은 `FR-SHELL-013` 이 막혀 뒤로 미룬다)
2. **다음 세션은 7·8·9 착수 전에 막힌 것 보고서를 도큐라이트로 먼저 보인다** — **확정**. (사용자 지시 2026-08-22)
...
12. **새 시험을 쓰면 대상 소스를 고의로 망가뜨려 그 시험이 죽는지 확인한다** — **확정**. (이번 세션에 이 방법으로 공허한 시험 셋과 실제 유출 하나를 찾았다)
...
- **`G36` ①** 검색 필터 팝오버의 기본값 — 네 대상(제목·본문·태그·첨부 이름) 중 처음에 무엇이 켜져 있는가. 심의 3인이 **전부 켜짐**을 지지했으나 원장에 기록하지 않았다. 결정 방법: 사용자 확인 또는 심의 결과 채택
- **`G36` ⑤** 텍스트 색인 갱신 시점 — 심의 3인이 **동기 갱신 + 존재하지 않는 노드는 결과에서 무조건 제외(fail-closed)** 를 지지했으나 원장에 기록하지 않았다
...
- **`G38`** 한국어 질의의 일치 단위가 `R149-c` 의 최소 2자와 충돌 — 실측으로 열렸다. `trigram` 은 3자 미만 질의를 0건으로 돌려주고, `unicode61` 은 `회의` 가 `회의록`에 안 걸린다(접두 매칭을 붙이면 어절 선두만 걸린다). 갈래 셋: ⓐ `unicode61` + 접두 자동 부착 ⓑ `trigram` + 2자는 `LIKE` 스캔 ⓒ `R149-c` 의 2자를 3자로 개정. **판정자 `adj-tokenizer` 의 최종 판정을 받지 못했다**
...
- **`CON-ACL-001` AC-4 와 청크 판정의 충돌** — AC-4 는 「한 요청의 권한 판정이 쿼리 2회」인데 검색이 청크 판정을 쓰면 질의가 청크 수만큼 늘어난다. 반대로 청크를 안 쓰면 **총 노드 32,766건에서 `too many SQL variables` 로 죽는다**(판정자 `adj-acl-sql` 실측: md 29,000 성공 / 30,000 실패). 둘 다 만족하는 구현이 없다. 결정 방법: `CON-ACL-001` 에 검색 표면용 AC 신설 또는 AC-4 의 예산 단위를 「판정 호출 1회당」으로 개정
...
- [ ] **E-3** `FR-SHELL-013` 구현 — 완료 조건: `blocked` 를 벗어나고 AC 전건이 재어진다. 추정 규모: 서버 신규 8파일 약 900~1100줄 + 시험 약 600줄 + 마이그레이션 1개 (판정자 3인의 추정치가 그 범위에 모였다)
...
### F. 증거 기록만 남은 것 — 조사가 끝났고 손에 답이 있다

- [ ] **F-1** `SEC-AUTH-002`~`SEC-AUTH-016` 15건 증거 — 완료 조건: 전 AC 덮임 10건(`003`·`006`·`007`·`008`·`009`·`011`·`012`·`013`·`015`·`016`)은 `verified`, 부분 5건(`002`·`004`·`005`·`010`·`014`)은 덮인 만큼만 기록. **조사 결과는 이 세션 대화에만 있고 저장소에 없다 — 다시 조사해야 한다**
- [ ] **F-2** `R21-a` 정정 — 완료 조건: 원장 조항의 「`WHERE` 절 하나로 성립」 주장이 정정됐고 인용 스윕 2곳(`<REPO>\docs\spec\02.product-architecture.srs.md` 의 `SEC-ARCH-002` Implementation Notes 와 Trace Links)이 함께 고쳐졌다
- [ ] **F-3** `G36` ③ 결과 상한 기록 — 완료 조건: 원장에 「랭크 순 청크 판정 후 통과분에만 상한 · 응답 DTO 에 적중 건수 칸을 두지 않는다」가 기록됐다
