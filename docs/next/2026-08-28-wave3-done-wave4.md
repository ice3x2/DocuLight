...
1. 이 문서를 끝까지 읽는다.
2. `git status --porcelain` 이 **비어 있고** `git log --oneline -1` 이 `d7736b4 chore(kiwi): wave-4 진입을 저널에 적는다` 인지 확인한다. 어긋나면 이 문서가 낡은 것이다.
...
- 미커밋 파일: **없음(clean).** 이 핸드오프 문서와 `docs/next/LATEST.md` 는 이 문서를 쓴 직후 커밋한다
- 이 문서 직전의 커밋: `d7736b4 chore(kiwi): wave-4 진입을 저널에 적는다`
