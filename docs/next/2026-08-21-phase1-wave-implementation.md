...
>
> **⚠ 검증되지 않은 축이 남는다** — 해석이 끼는 주장(§5 확정 결정의 타당성, §8 지시서가
> 실제로 실행 가능한지, §9 함정 서술의 정확성)은 **독립 검증을 받지 못했다.** 자기 문서를
> 자기가 판정한 것은 계수뿐이다. 다음 세션은 §5·§8 을 **읽되 그 근거 문서를 함께 열어라** —
> 각 항목이 어느 문서를 가리키는지 §4 에 적혀 있다.
