...
> **사실 대조 상태 (2026-08-21)** — 이 문서의 **수치·경로·git·설정 주장 59건을 스크립트로
> 실측 대조해 전건 참**이다(경로 27개 실존 · wave 9 · 요구 250 · 조항 350 · 커버리지
> 329+6+15 · 살아 있는 조항 346 미매핑 0 · 계획 9 Phase 53 Task · AC 118 커버 111 유예 7 ·
> Phase 별 Task 수 9행 · trace 간선 264 · 커밋 11개 실존 · 도메인 인프라 import 0 ·
> 포트 3399 · `NODE_ENV=production`).
>
> **독립 검증도 받았다** — `docs/analysis/handoff-factcheck-2026-08-21.md` ·
> **TRUE 76 · FALSE 5 · UNVERIFIABLE 6.** FALSE 다섯을 전건 처분했다(Task 수 커밋 귀속 ·
> push tip · 워킹트리 상태 · `OPS-STORAGE-001` 과 케이스 폴딩이 이미 결정돼 있었음).
> 그리고 검증이 **두 산출물의 값이 갈린 자리 하나**를 찾았다 — §8 의 경고를 보라.
>
> **UNVERIFIABLE 6 은 저장소 밖 사실이다** — 담당자 3인의 작업 격리 여부, 과거 세션에서
> 겪은 함정(④⑤⑦⑧), `B:` 드라이브의 실체. **참이라고 단정하지 말고 참고로 읽어라.**
...
- [x] **wave-1 계획** — 9 Phase · **55 Task** · AC 118개 중 커버 111 · 유예 7 (`0637c1e`)
- [x] **계획 독립 검증 + HIGH 3 처분** — `5a541b2`. 이때 **55 → 53 Task** 가 됐다 (H-3 처분으로 `T-PH003-03/04` 가 자기 AC 를 잃어 `05/06` 에 흡수됨. 번호는 재사용하지 않았다)
...
- 브랜치 `master`. 원격 `origin` = `B:/work/git/DocuLight2.0.git` (로컬 백업)
- **워킹트리는 클린이고 origin 과 동기화돼 있다** (ahead 0 · behind 0)
- **`git status --porcelain` 과 `git log --oneline -1` 을 직접 읽어라** — 이 절은 작성 시점
  스냅샷이라 **반드시 낡는다.** 실제로 초판이 두 자리에서 낡았고 검증이 그것을 잡았다
...
9. **`FR-WORKSPACE-004` 세 값**: 금지 문자 = Win32 예약 문자 9 + 제어문자 + 말단 공백·마침표 / 최대 이름 길이 = **UTF-8 255바이트** / 경로 총길이 = **워크스페이스 루트 기준 상대 UTF-8 512바이트**
   > **255 가 아니라 512 다.** 초판이 255 로 적었고 그 근거(*"4단계 계층 상한이 깊이를 묶으므로 넉넉하다"*)가 `CON-WORKSPACE-001` 판정으로 무너졌다. 값은 AC 자체에서 도출된다 — **경로 상한이 이름 상한의 두 배에 못 미치면 `AC-5` 가 허용하는 이름을 `AC-6` 이 거부하는 자기모순**이 생긴다(상한 길이 디렉토리 아래에 상한 길이 파일을 둘 수 없다). `255 × 2 + 2` = **512** 가 두 AC 를 동시에 성립시키는 최소값이며 그보다 큰 값을 고를 근거는 원장에 없다. 상한의 목적은 **이식성 보장이 아니라 병리적 입력 차단**이다 — 이식성은 원리적으로 보장 불가다(계획 `RISK-03`). 논증 전문은 `plan.md:3256~3273`
...
> **⚠ 초판이 아래 둘을 미결로 적었으나 계획이 이미 결정해 두었다**(검증 지적). 사이드카
> `open_questions` 를 보라 — 미결 목록을 읽기 전에 그쪽을 먼저 확인하는 것이 옳다.
>
> - **`OPS-STORAGE-001`** (`OQ-03`) — *"둘로 가른다 — AC-1·AC-4 는 운영 런북(문서)으로,
>   AC-2·AC-3 은 자동 테스트로 닫는다."* 초판의 *"자동 테스트로 닫히지 않는다"* 는
>   **AC-2·AC-3 에 대해 거짓**이다. PH-008 은 결정 지점이 아니라 그 결정의 **실행**(런북 작성)이다
> - **비-ASCII 케이스 폴딩** (`OQ-06`) — *"로케일 독립 유니코드 기본 케이스 폴딩을 쓴다.
>   `toLowerCase` 를 쓰지 않는다."* PH-004 는 그 결정의 **구현**이다

- **`FR-WORKSPACE-004` 경로 총길이** — **512 로 정정됐다**(아래 §5-9 참조). 초판이 255 로
  적었고 그 근거가 무너져 계획의 512 논증을 채택했다. **낡은 255 를 인용하지 마라**
...
> **⚠ `T-PH004-05` 를 쓰기 전에 읽어라 — 두 산출물이 어긋나 있었다.**
>
> 사이드카 `TC-REQ-FR-WORKSPACE-004-AC6-01` 의 `expected_failure_signature` 가
> *"relative path over **512** UTF-8 bytes accepted"* 이고, 요구사항 기록의 초판은
> **255** 였다. **512 가 맞다**(§5-9). 요구사항 쪽을 2026-08-21 에 정정했으므로
> 이제 둘이 같다 — 다만 **낡은 255 를 인용한 자리가 더 있는지 확인하고 쓰라.**
>
> 이 자리는 검증이 잡았다. **아래 「각 Task 의 정확한 명세는 사이드카에 있다」와
> §5 의 값이 갈릴 수 있다** — 갈리면 **요구사항 기록(`npx speckiwi show`)이 SSOT** 이고,
> 그 기록이 틀렸다고 판단되면 고친 뒤 진행하라. 이번이 그런 경우였다.
