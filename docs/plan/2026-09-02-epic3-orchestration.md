...
### 8.8 어휘로 재는 방벽은 재수출 한 줄로 우회된다 (2026-09-02 실측)

이슈 #29 가 「MCP 표면의 소스가 `issueToken`·`revokeToken`·`listTokens` 를 부르지
않는다」는 항을 세웠다. 도구 카탈로그를 전량으로 못박는 대신 고른 방식이고, 그
근거는 **도구가 늘 때마다 목록을 고치게 되면 고치는 사람이 자격 재생산 여부를
판단하지 않는다**는 것이었다.

**그 방벽은 재수출 한 줄로 우회된다.** 검증자가 직접 만들어 확인했다.

```
export { issueToken as mintToken }
```

이 한 줄을 다른 디렉터리에 두고 dispatch 가 그 이름을 부르면, **MCP 도구로 PAT 를
발급하는 코드가 살아 있는 채 전건이 통과하고 typecheck 도 지난다.** 훑는 두 자리
밖에 새 표면 파일을 세우는 우회도 같다.

**그럼에도 그 방식을 유지한다.** 전량 목록은 유지 비용이 매번 들면서 같은 우회에
똑같이 뚫리고, 진짜 호출 그래프를 세우려면 컴파일러 API 가 필요해 이 회차의
범위를 넘는다.

**기록하는 이유는 방벽의 한계를 아는 것이 방벽을 믿는 것보다 낫기 때문이다.**
이 방벽은 「실수로 붙이는 것」을 막지 「우회하려는 것」을 막지 못한다.
