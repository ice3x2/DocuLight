...
- **`packages/web` 에도 브라우저 검사가 있다** (2026-08-29 신설). editor 의 검사와 전제가
  ...
  **수용 기준 3**의 `[[` 자동완성)이며, **진짜 입력 제스처가 핸들러까지 닿는가**를 잰다 —
  ...
  로그아웃 · 비밀번호 변경, 그리고 **가입 신청·승인**의 왕복)이며, **`_web-harness.mjs`
  의 `login` 지름길을 쓰지 않는 유일한 검사**다 — 그 함수는 API 로 세션을 세우는데 여기서는 로그인 화면 자체가
  ...
  **로그인은 15분 창에 10회로 제한되고 web 검사 아홉이 한 번에 16회를 쓴다**(다섯째가
  둘, 아홉째가 일곱) — **한 창에 전부를 돌리지 못한다.** 창이 열릴 때까지 기다리거나
