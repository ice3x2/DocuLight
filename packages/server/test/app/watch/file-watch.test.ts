...
describe('파일 감시가 상관 판정을 돌린다', () => {
  it('AC-1: 서버에서 직접 옮긴 파일이 같은 노드로 인정된다', async () => {
    ...
    // **창 값 자체는 여기서 재지 않는다** — 그 값이 짧다는 것은
    // `test/domain/watch/correlation.test.ts` 가 상한으로 붙들고, 이 파일은
    // 사건이 판정까지 이어지는지만 본다.
    //
    // 그렇더라도 창은 **두 사건의 도착 간격보다 넓어야** 한다. 이 항이 재는
    // 사슬은 개명 하나가 만드는 `unlink` 와 `add` 가 같은 묶음으로 판정될
    // 때에만 이어지는데, 판정은 마지막 사건에서 창만큼 더 기다렸다가 돌기
    // 때문이다 (`file-watch.ts` 의 `schedule`). 앞의 것이 창 안에 짝을 만나지
    // 못하면 짝 없는 `unlink` 로 tombstone 이 되고, 뒤늦게 온 `add` 는 신규
    // 노드가 된다.
    //
    // 처음 쓴 값 120ms 는 그 간격의 분포 안에 있었다. 놀고 있는 기계에서는
    // p50 68ms · 최대 89ms 라 걸리지 않지만 부하를 걸면 꼬리가 창 밖으로
    // 나간다 — CPU 16워커에서 서른 번 중 한 번이 120ms 를 넘었고(최대
    // 175ms), 전체 회귀가 함께 도는 부하에서는 최대 246ms 까지 관측됐다
    // (2026-09-02 실측). 그래서 이 항이 열두 회차 인구조사에서 한 번 죽었다.
    //
    // **값은 위아래로 붙들려 있다.** 아래로는 관측된 최대 간격 246ms 보다
    // 넉넉해야 하고, 위로는 `seen()` 이 스스로 세우는 상한
    // (`windowMs * 20 + 2_000`)이 이 스위트의 `testTimeout` 20초를 넘지
    // 않아야 한다. 넘기면 사건이 아예 오지 않는 결함이 단언 메시지 없이
    // 타임아웃으로 죽고, 그때 `afterEach` 의 정리(감시자 정지 · DB 닫기 ·
    // 임시 디렉터리 제거)가 건너뛰어져 남은 상태가 다음 항까지 넘어뜨린다.
    // 그 상한이 창을 900ms 아래로 묶는다. 800ms 는 관측 최대의 3.2 배이면서
    // 그 안이다.
    //
    // **단언은 하나도 무르게 하지 않는다** — 넓힌 것은 언제까지 기다리는가이지
    // 무엇을 요구하는가가 아니며, 사슬이 끊기면 경로가 그대로라 여전히 죽는다.
    // 아래 두 항이 120ms 를 그대로 쓰는 이유는 그쪽이 재는 것이 사건을
    // **받는 시점**의 거르기여서 묶음의 폭과 무관하기 때문이다.
    watch = await startFileWatch(stores, docsRoot, { windowMs: 800 });
    await watch.ready();
