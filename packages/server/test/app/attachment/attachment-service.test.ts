...
describe('DR-ATTACH-003 — 본문 링크는 워크스페이스 기준 절대경로다', () => {
  ...
  it('DR-ATTACH-003 AC-2 · FR-STORAGE-008 AC-1 · AC-2: 문서를 옮겨도 첨부 경로와 본문 바이트가 그대로다', async () => {
    ...
    // 이동 전 자리의 바이트를 본다. `moveNode` 는 노드 행만 옮기고 실물은
    // 건드리지 않으며, 디스크를 따라가게 하는 것은 라우트의 몫이다
    // (`FR-SHELL-015` — `POST /nodes/:id/move`). 이 AC 가 말하는 것은
    // 「서버가 이동을 이유로 본문을 고쳐 쓰지 않는다」이므로 서비스만
    // 부르는 여기서는 옮기기 전 자리가 그대로여야 한다.
