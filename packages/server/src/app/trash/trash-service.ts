...
export async function moveToTrash(
  ...
  // **DB 를 먼저, 디스크를 나중에** (`REL-STORAGE-003` AC-1). 이름 변경·이동이
  // 이미 그 순서인데 삭제만 뒤집혀 있었다.
  //
  // 감시는 파일이 원래 자리에서 사라지는 것을 보고, 그 노드가 휴지통에
  // 들었는지로 「서버가 옮긴 것」과 「누가 지운 것」을 가른다. 파일을 먼저
  // 옮기면 그 판정이 도는 순간 아직 표시가 없어 짝 없는 사라짐이 되고 노드가
  // 고아가 된다. 복구는 휴지통 표시만 지우므로 고아 표시가 남아, 되살린
  // 문서가 트리에는 서고 열리지 않는다 — 사용자는 트리에 이름이 있으니
  // 잃어버린 줄도 모른다.
  //
  // 뒤엣것이 실패하면 표시만 남는다. 그 상태는 트리에서 사라지고 휴지통에
  // 보이므로 사용자가 무슨 일이 났는지 볼 수 있다 — 반대 순서의 실패(파일만
  // 사라지고 아무 데도 안 보임)보다 낫다.
