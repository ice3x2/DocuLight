...
-- 범위 — 이 트리거들은 **앞으로 들어오는 쓰기**만 막는다. 이미 위반
-- 상태로 들어와 있는 행을 검사하거나 수리하지 않는다. 007 까지의 정상
-- 경로로는 그런 데이터가 생길 수 없으므로 백필을 두지 않았다. 만약 손으로
-- 만진 DB 가 있다면 그 사실은 이 마이그레이션이 알려 주지 못한다.
