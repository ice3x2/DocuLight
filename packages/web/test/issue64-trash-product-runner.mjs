...
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'doculight-issue64-product-'));
...
try {
  ...
  const userName = `issue64-${randomUUID()}`; const password = `Issue64-${randomUUID()}`;
  const installed = await fetch(new URL('/api/install/commit', origin), { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ installSession: verified.installSession, superuserName: userName, password, workspaceName: 'Issue64 isolated product', defaultGroupLevel: 'none', signupMode: 'approval' }) });
  ...
  const managerName = `issue64-manager-${randomUUID()}`; const managerPassword = `Issue64-manager-${randomUUID()}`;
  const emptyName = `issue64-empty-${randomUUID()}`; const emptyPassword = `Issue64-empty-${randomUUID()}`;
  ...
  const editedWorkspace = await createWorkspace({ workspaces: runtime.stores.workspaces, files: runtime.stores.files }, 'Issue64 edit-only workspace');
  ...
  await child(path.join(webRoot, 'test/issue64-trash-product-check.mjs'), {
    ...process.env,
    WEB_URL: origin,
