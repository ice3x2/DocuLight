...
try {
  ...
  for (const [width,height,theme,zoom] of compositionEnvironments) {
    ...
    await trashControl.fill('1');
    await auditControl.fill('1');
    await form.getByRole('button',{name:'저장'}).click();
    const retention=page.getByRole('alertdialog'); await retention.waitFor();
    const token=retention.getByTestId('retention-impact-token'); await token.waitFor();
    assert.equal(await token.count(),1,JSON.stringify({retentionBaseline,dialog:await retention.innerText(),guardedRequests:guardedRequests.slice(-4)}));
    const beforeRetention=guardedRequests.length;
    ...
    assert.equal(retentionPutAfter-retentionPutBefore,1); assert(retentionGetAfter-retentionGetBefore>=1); assert.equal(await trashControl.inputValue(),'1');
    ...
    await trashControl.fill(retentionBaseline.trash); await auditControl.fill(retentionBaseline.audit);
    const restorePutBefore=settingsRequests.filter((request)=>request.method==='PUT').length;
    const restoreSave=form.getByRole('button',{name:'저장'}); await restoreSave.focus(); await restoreSave.press('Enter');
    for(let attempt=0;attempt<100&&settingsRequests.filter((request)=>request.method==='PUT').length===restorePutBefore;attempt+=1) await page.waitForTimeout(25);
    assert.equal(settingsRequests.filter((request)=>request.method==='PUT').length-restorePutBefore,1);
    await page.waitForFunction(([trash,audit])=>{const fields=[...document.querySelectorAll('[data-instance-settings] input')];return fields.some((node)=>node.value===trash)&&fields.some((node)=>node.value===audit);},[retentionBaseline.trash,retentionBaseline.audit]);
    ...
    await leaveFieldForComposition.evaluate((node)=>new Promise((resolve,reject)=>{const deadline=performance.now()+2000;const frame=()=>{if(document.activeElement===node)return resolve(true);if(performance.now()>=deadline)return reject(new Error('focus return timeout'));requestAnimationFrame(frame);};requestAnimationFrame(frame);}));
    const retainedDraft=await leaveFieldForComposition.inputValue(); const retainedFocus=await leaveFieldForComposition.evaluate((node)=>document.activeElement===node); assert.equal(retainedDraft,leaveDraftValue); assert.equal(retainedFocus,true);
