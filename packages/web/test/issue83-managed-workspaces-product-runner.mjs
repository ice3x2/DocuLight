...
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'doculight-issue83-product-'));
let runtime;
let server;
let zeroRuntime;
let zeroServer;
...
try {
  ...
  const ownerName = `issue83-owner-${randomUUID()}`;
  const ownerPassword = `Issue83-owner-${randomUUID()}`;
  const committed = await fetch(new URL('/api/install/commit', origin), {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ installSession: verified.installSession, superuserName: ownerName, password: ownerPassword, workspaceName: 'Issue83 managed workspace', defaultGroupLevel: 'none', signupMode: 'approval' }),
  });
  ...
  const makeAccount = async (prefix) => {
    ...
    const password = `Issue83-${prefix}-${randomUUID()}`;
  ...
  const zeroPassword = `Issue83-zero-${randomUUID()}`;
  ...
  await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [path.join(webRoot, 'test/issue83-managed-workspaces-product-check.mjs')], {
      cwd: webRoot,
      stdio: 'inherit',
      env: {
        ...process.env,
        WEB_URL: zeroOrigin,
        DOCULIGHT_ISSUE83_MANAGED_URL: origin,
        DOCULIGHT_ISSUE83_OWNER_USER: ownerName,
        DOCULIGHT_ISSUE83_OWNER_PASS: ownerPassword,
        DOCULIGHT_ISSUE83_ZERO_USER: zeroName,
        DOCULIGHT_ISSUE83_ZERO_PASS: zeroPassword,
        DOCULIGHT_ISSUE83_MANAGER_USER: manager.name,
        DOCULIGHT_ISSUE83_MANAGER_PASS: manager.password,
        DOCULIGHT_ISSUE83_ORDINARY_USER: ordinary.name,
        DOCULIGHT_ISSUE83_ORDINARY_PASS: ordinary.password,
      },
    });
