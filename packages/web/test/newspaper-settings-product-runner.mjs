...
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'doculight-issue61-product-'));
let runtime;
let server;
let primaryError;
try {
  ...
  const username = `issue61-${randomUUID()}`;
  const password = `Issue61-${randomUUID()}`;
  ...
  await runChild(process.execPath, [path.join(webRoot, 'test/newspaper-settings-product-check.mjs')], {
    cwd: webRoot,
