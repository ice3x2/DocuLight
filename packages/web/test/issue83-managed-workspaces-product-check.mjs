...
const output = path.join(root, '.kiwi/sessions/newspaper-20260916/evidence/issue83/browser-matrix');
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'doculight-issue83-browser-'));
...
fs.writeFileSync(path.join(extension, 'manifest.json'), JSON.stringify({ manifest_version: 3, name: 'Issue 83 zoom controller', version: '1.0.0', permissions: ['tabs'], background: { service_worker: 'worker.js' } }));
...
const owner = [required('DOCULIGHT_ISSUE83_OWNER_USER'), required('DOCULIGHT_ISSUE83_OWNER_PASS')];
const zero = [required('DOCULIGHT_ISSUE83_ZERO_USER'), required('DOCULIGHT_ISSUE83_ZERO_PASS')];
const manager = [required('DOCULIGHT_ISSUE83_MANAGER_USER'), required('DOCULIGHT_ISSUE83_MANAGER_PASS')];
const ordinary = [required('DOCULIGHT_ISSUE83_ORDINARY_USER'), required('DOCULIGHT_ISSUE83_ORDINARY_PASS')];
const managedUrl = required('DOCULIGHT_ISSUE83_MANAGED_URL');
const calls = [];
const captures = [];
...
const closureEvidence = [
  { path: 'docs/spec/13.workspace.srs.md', sha256: hashLfText(path.join(root, 'docs/spec/13.workspace.srs.md')), normalization: 'LF' },
  { path: '.kiwi/sessions/newspaper-20260916/evidence/issue83/decision-review.md', sha256: hashLfText(path.join(root, '.kiwi/sessions/newspaper-20260916/evidence/issue83/decision-review.md')), normalization: 'LF' },
];
const readManagedScope = async (page, role) => {
  const result = await page.evaluate(async () => {
    const response = await fetch('/api/managed-workspaces');
    return { status: response.status, cacheControl: response.headers.get('cache-control'), body: await response.json() };
  });
  managedScopes.push({ role, ...result });
  assert.equal(result.status, 200);
  assert.equal(result.cacheControl, 'private, no-store');
  assert.deepEqual(Object.keys(result.body).sort(), ['scope', 'workspaces']);
  assert.equal(result.body.workspaces.every((row) => Object.keys(row).sort().join(',') === 'id,name'), true);
  return result.body;
};
...
try {
  ...
  const ownerScope = await readManagedScope(page, activeRole);
  assert.equal(ownerScope.scope, 'instance');
  assert.ok(ownerScope.workspaces.length >= 1);
  ...
  const zeroScope = await readManagedScope(page, activeRole);
  assert.deepEqual(zeroScope, { scope: 'instance', workspaces: [] });
  await page.getByRole('button', { name: '설정', exact: true }).first().click();
  ...
  const ordinaryScope = await readManagedScope(page, activeRole);
  assert.deepEqual(ordinaryScope, { scope: 'managed-workspaces', workspaces: [] });
  await page.getByRole('button', { name: '설정', exact: true }).first().click();
  ...
  const managerScope = await readManagedScope(page, activeRole);
  assert.equal(managerScope.scope, 'managed-workspaces');
  assert.equal(managerScope.workspaces.length, 1);
  await page.getByRole('button', { name: '설정', exact: true }).first().click();
  ...
  const manifest = {
    ...
    authorityTransition: {
      ...
      integrationTest: 'packages/web/test/issue83-managed-workspaces.test.tsx',
      integrationSourceSha256: hashFile(path.join(root, 'packages/web/test/issue83-managed-workspaces.test.tsx')),
  ...
  process.stdout.write('PASS issue83 product: role/privacy path + 12 normal and 2 forced-color captures\n');
} finally {
