 ...
 * 로그인한 뒤의 화면이고, 화면을 거치면 그 절차가 매 검사의 앞에 붙어 실패
 * 지점이 흐려진다.
 *
 * **로그인 화면 자체를 재는 검사는 이것을 쓰지 않는다.** `auth-round-trip-check`
 * 는 그 화면이 대상이라 이 지름길을 쓸 수 없고, 실제로 2026-09-01 이전에는
 * 그 화면이 제목만 있는 자리표였는데도 나머지 검사가 전부 통과했다.
 */
