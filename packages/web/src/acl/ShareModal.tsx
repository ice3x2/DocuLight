...
export type ShareActionResult = { readonly ok: true } | { readonly ok: false };
...
export function ShareModal({
  ...
  const [revokeTarget, setRevokeTarget] = useState<{ row: ShareRow; warnings: readonly GrantWarning[] } | null>(null);
  const [pending, setPending] = useState(false);
  const [preparing, setPreparing] = useState(false);
  const [result, setResult] = useState<{ kind: ActionKind; ok: boolean; subject?: string; entryId?: string; principalId?: string; requestedLevel?: 'view' | 'edit'; identityUnavailable?: boolean } | null>(null);
  ...
  const safeAction = async (kind: ActionKind, callback: (() => Promise<ShareActionResult>) | undefined, subject?: string, deferResult = false) => {
    if (callback === undefined || pending) return false;
    ...
    if (deferResult && outcome?.ok === true) return true;
    setPending(false);
  ...
  const resolveGrantMetadata = async (principalId: string, subject: string, requestedLevel: 'view' | 'edit', generation: number) => {
    ...
    setPending(false);
    setResult({ kind: 'grant', ok: true, subject, principalId, requestedLevel, ...(current?.rows === null ? { identityUnavailable: true } : {}), ...(authoritative?.entryId === undefined || authoritative.entryId === null ? {} : { entryId: authoritative.entryId }) });
    return true;
  };

  const grantNow = async (principal: PrincipalRow, grantLevel: 'view' | 'edit') => {
    const ok = await safeAction('grant', onGrant === undefined ? undefined : () => onGrant(principal.id, grantLevel), principal.name, true);
    if (ok) {
      const generation = operation.current;
      if (!await resolveGrantMetadata(principal.id, principal.name, grantLevel, generation)) return false;
      setSelected(null);
    }
    return ok;
  };
  ...
  const revoke = async (row: ShareRow) => {
    ...
    const ok = await safeAction('revoke', onRevoke === undefined ? undefined : () => onRevoke(row.entryId!), row.principalName);
    if (ok || ready?.nodeKind === 'file') setRevokeTarget(null);
  };
  ...
  const startRevoke = async (row: ShareRow) => {
    ...
    if (operation.current !== generation) return;
    if (warnings === undefined) {
      setResult({ kind: 'revoke', ok: false });
      return;
    }
    setRevokeTarget({ row, warnings });
  };
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content data-share-dialog aria-labelledby={titleId} onOpenAutoFocus={(event) => {
          ...
          <div data-share-body>
            ...
            {ready === undefined ? null : <>
              ...
              <section data-share-grant>
                <PrincipalPicker scope={`node:${nodeId}`} onPick={(principal) => { operation.current += 1; setPreparing(false); setSelected(principal); }} onSelectionInvalidated={() => { operation.current += 1; setPreparing(false); setSelected(null); }} />
              ...
              {result?.ok && result.kind === 'grant' && result.subject !== undefined ? <>
                <GrantToast subjectName={result.subject} {...(result.identityUnavailable ? { limitation: '현재 권한에서는 회수 대상을 확인할 수 없습니다.' } : {})} {...(result.entryId === undefined ? {} : { entryId: result.entryId, onRevoke: (entryId: string) => { void startRevoke({ entryId, principalId: '', principalName: result.subject!, principalKind: 'user', level: 'view', inherited: false, source: null }); } })} />
          ...
          <RevokeConfirm
            ...
            targetKind={ready?.nodeKind ?? 'file'}
            subjectName={revokeTarget?.row.principalName ?? ''}
            forceConfirm={(revokeTarget?.warnings.length ?? 0) > 0}
            onConfirm={async () => { if (revokeTarget !== null) await revoke(revokeTarget.row); }}
